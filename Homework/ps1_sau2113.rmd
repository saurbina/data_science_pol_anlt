...
sum(is.na(fec22.df$V21))
sum(is.na(fec22.df$V22))
sum(is.na(fec22.df$V23))
sum(is.na(fec22.df$V24))
sum(is.na(fec22.df$V25))
...
#Cheking the subsets for correct outcome
describe(sen_df$V20)
summary(sen_df$V20)
sum(sen_df$n)
...
```{r, include=TRUE}
# Calculating the mean, median and sd for Senate and House using dplyr and the original data, for this we need to group the data by V20 == 0, which is the variable that indicates the chamber and all the observations that not equal to 0 are House candidates.
print(fec22.df %>%
group_by(V20==0) %>%
...
p1 + scale_x_continuous(limits = c(0, 20))
